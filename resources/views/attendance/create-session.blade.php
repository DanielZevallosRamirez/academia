...
@section('title', 'Nueva Sesion de Clase')
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-slate-800">Nueva Sesion de Clase</h1>
        <p class="text-slate-500 mt-1">Crea una nueva sesion para registrar asistencia</p>
    </div>

    {{-- Formulario --}}
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <form action="{{ route('attendance.store-session') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title') }}" placeholder="Ej: Clase 1 - Introduccion" required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="location" id="location" value="{{ old('location') }}" placeholder="Ej: Aula 101"
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" placeholder="Notas adicionales sobre la sesion..."
                    class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors resize-none @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
