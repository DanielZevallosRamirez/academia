...
@php
    ...
    $roleIcons = ['admin' => 'shield', 'profesor' => 'book-open', 'estudiante' => 'graduation-cap'];
@endphp
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
            @if($user->role === 'estudiante')
                <a href="{{ route('users.qr', $user) }}" class="px-4 py-2 border border-gray-200 rounded-lg hover:bg-gray-50 flex items-center gap-2">
                    <i data-lucide="qr-code" class="w-4 h-4"></i>
                    Ver QR
                </a>
            @endif
            <a href="{{ route('users.edit', $user) }}" class="px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-xl border border-gray-200 p-6">
                <div class="text-center">
                    ...
                    <h2 class="mt-4 text-xl font-bold text-gray-900">{{ $user->name }} {{ $user->last_name }}</h2>
                    
                    <!-- Role Badge -->
                    <span class="inline-flex items-center gap-1 px-3 py-1 mt-2 text-sm font-medium rounded-full bg-{{ $roleColors[$user->role] }}-100 text-{{ $roleColors[$user->role] }}-700">
                        <i data-lucide="{{ $roleIcons[$user->role] }}" class="w-4 h-4"></i>
                        {{ $roleNames[$user->role] }}
                    </span>
                ...
                <div class="mt-6 space-y-4">
                    ...
                    @if($user->dni)
                        <div class="flex items-center gap-3 text-sm">
                            ...
                            <span class="text-gray-600">DNI: {{ $user->dni }}</span>
                        </div>
                    @endif
                    @if($user->birth_date)
                        <div class="flex items-center gap-3 text-sm">
                            <i data-lucide="calendar" class="w-5 h-5 text-gray-400"></i>
                            <span class="text-gray-600">{{ $user->birth_date->format('d/m/Y') }}</span>
                        </div>
                    @endif
                    @if($user->address)
                    ...
                    @endif
                    @if($user->role === 'profesor' && $user->specialty)
                        <div class="flex items-center gap-3 text-sm">
                            <i data-lucide="briefcase" class="w-5 h-5 text-gray-400"></i>
                            <span class="text-gray-600">{{ $user->specialty }}</span>
                        </div>
                    @endif
                </div>

                @if($user->role === 'estudiante' && $user->emergency_contact)
                    <div class="mt-6 pt-6 border-t border-gray-200">
                        <h4 class="text-sm font-medium text-gray-900 mb-2">Contacto de Emergencia</h4>
                        <p class="text-sm text-gray-600">{{ $user->emergency_contact }}</p>
                        @if($user->emergency_phone)
                            <p class="text-sm text-gray-500">{{ $user->emergency_phone }}</p>
                        @endif
                    </div>
                @endif
                ...
                @if($user->role === 'estudiante')
                    <div class="mt-6 pt-6 border-t border-gray-200 grid grid-cols-2 gap-4">
                        <div class="text-center">
                            <p class="text-2xl font-bold text-gray-900">{{ $user->getAttendanceRate() }}%</p>
                            <p class="text-xs text-gray-500">Asistencia</p>
                        </div>
                        <div class="text-center">
                            <p class="text-2xl font-bold text-gray-900">{{ $user->getPendingPaymentsCount() }}</p>
                ...
                <!-- Account Info -->
                <div class="mt-6 pt-6 border-t border-gray-200">
                    <h4 class="text-sm font-medium text-gray-900 mb-2">Informacion de Cuenta</h4>
                    <div class="space-y-2 text-sm">
                        ...
                        @if($user->email_verified_at)
                            <div class="flex justify-between">
                                ...
                                <span class="text-green-600">Si</span>
                            </div>
                        @else
                            <div class="flex justify-between">
                                <span class="text-gray-500">Email verificado</span>
        ...
        <div class="lg:col-span-2 space-y-6">
            @if($user->role === 'estudiante')
                    ...
                    <div class="divide-y divide-gray-100">
                        ...
                        @empty
                            <div class="p-8 text-center text-gray-500">
                                <i data-lucide="book-open" class="w-12 h-12 mx-auto text-gray-300 mb-2"></i>
                                <p>Sin inscripciones</p>
                                <a href="{{ route('users.edit', $user) }}" class="text-sm text-emerald-600 hover:text-emerald-700">
                ...
                <div class="bg-white rounded-xl border border-gray-200">
                    <div class="p-6 border-b border-gray-200 flex items-center justify-between">
                        ...
                        <a href="{{ route('payments.create', ['student_id' => $user->id]) }}" 
                ...
                <div class="bg-white rounded-xl border border-gray-200">
                    ...
                    <div class="divide-y divide-gray-100">
                        @forelse($user->attendances->take(5) as $attendance)
                            <div class="p-4 flex items-center gap-4">
                                ...
                                <div class="flex-1">
                                    <p class="font-medium text-gray-900">{{ $attendance->classSession->title ?? 'Sesion' }}</p>
            ...
            @elseif($user->role === 'profesor')
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Informacion del Profesor</h3>
                    <div class="grid grid-cols-2 gap-4">
                        <div class="p-4 bg-blue-50 rounded-lg text-center">
                            <i data-lucide="book-open" class="w-8 h-8 mx-auto text-blue-600 mb-2"></i>
                            <p class="text-2xl font-bold text-blue-600">0</p>
                            <p class="text-sm text-gray-600">Cursos asignados</p>
                        </div>
                        <div class="p-4 bg-emerald-50 rounded-lg text-center">
                            <i data-lucide="users" class="w-8 h-8 mx-auto text-emerald-600 mb-2"></i>
                    ...
                    @if($user->specialty)
                        <div class="mt-4 p-4 bg-gray-50 rounded-lg">
                            ...
                            <p class="font-medium text-gray-900">{{ $user->specialty }}</p>
                        </div>
                    @endif
                </div>
            @else
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Informacion del Administrador</h3>
                    <div class="p-4 bg-purple-50 rounded-lg">
                        <div class="flex items-center gap-3">
                            <i data-lucide="shield-check" class="w-8 h-8 text-purple-600"></i>
                            <div>
                                <p class="font-medium text-gray-900">Acceso completo</p>
                                <p class="text-sm text-gray-600">Este usuario tiene permisos de administrador</p>
                            </div>
                        </div>
                    </div>
                </div>
